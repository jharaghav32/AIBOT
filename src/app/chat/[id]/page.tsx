...
import { BsBookmark, BsFillBookmarkFill,BsSun,BsMoon } from 'react-icons/bs'
...
const page = ({params:{id}}:Props) => {
  ...
  const bookMark = async () => {
    const bookmarkDoc = doc(db, 'users', session?.user?.email!, 'bookmark', id);
  ...
  const handleBookmark = async () => {
    try {
      ...
      const bookmarkDocRef = doc(db, 'users', userEmail, 'bookmark',id);
      const bookmarkDocSnap = await getDoc(bookmarkDocRef);

      if (bookmarkDocSnap.exists()) {
        // Bookmark exists, so remove it
        await deleteDoc(bookmarkDocRef);
        console.log(`Chat ID ${id} removed from bookmark successfully`);
      } else {
        // Bookmark doesn't exist, so add it
        await setDoc(bookmarkDocRef, {
          createdAt: new Date()
        });
        console.log(`Chat ID ${id} added to bookmark successfully`);
      }
    } catch (error) {
      console.error(error);
    }
